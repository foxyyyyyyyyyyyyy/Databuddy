...
export function useAllChartPreferences() {
    const [storedPreferences, setStoredPreferences] = usePersistentState<
        AllPreferences
    >(CHART_PREFERENCES_STORAGE_KEY, getDefaultPreferences());
    ...
    const updateLocationPreferences = (
        ...
        setStoredPreferences((prev) => {
            const current = isValidPreferences(prev)
                ? prev
                : getDefaultPreferences();
            return {
                ...current,
